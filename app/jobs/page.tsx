...
export default async function JobsPage({ searchParams }: { searchParams: Promise<JobsSearchParams> }) {
  ...
  const filteredJobs = jobRepository.filter(title || undefined, company || undefined, modality, location || undefined);
    ...
    <main className="container mx-auto p-4">
      <div className="mb-6 grid gap-4 xl:grid-cols-[1.5fr_1fr]">
        <div className="space-y-4">
          <div>
            <Link href="/jobs" className="text-2xl font-bold text-slate-900 hover:text-slate-700">
              Listado de empleos
            </Link>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <span className="text-sm text-slate-500">Página {page} / {lastPage}</span>
        <div className="flex items-center gap-2 text-sm">
          <Link href={buildJobsUrl({ title: title ?? '', company: company ?? '', modality: modality ?? '', location: location ?? '', limit, offset: previousOffset })} className="rounded-md border border-slate-200 bg-white px-3 py-2 text-slate-700 transition hover:bg-slate-50">
            Anterior
          </Link>
          <Link href={buildJobsUrl({ title: title ?? '', company: company ?? '', modality: modality ?? '', location: location ?? '', limit, offset: nextOffset })} className="rounded-md border border-slate-200 bg-white px-3 py-2 text-slate-700 transition hover:bg-slate-50">
            Siguiente
          </Link>
        </div>
      </div>
      ...
      <ul className="grid gap-4 sm:grid-cols-2">
        {jobs.map(job => (
          <li key={job.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
            <Link href={`/jobs/${job.id}`} className="block">
              <div className="flex flex-col gap-2">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">{job.modality}</span>
                </div>
                <p className="text-sm text-slate-600">{job.location}</p>
                <p className="text-sm text-slate-700">{job.salary.min.toLocaleString()} - {job.salary.max.toLocaleString()} {job.salary.currency}</p>
              </div>
            </Link>
          </li>
        ))}
